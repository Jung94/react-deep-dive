...
## Batching
Batching(배칭)은 여러 개의 상태 업데이트를 하나로 묶어 처리하여 렌더링 횟수를 줄이는 최적화 기법입니다.
...
1. Render Phase(렌더 단계)에서는 Reconciliation을 수행하여 새로운 Fiber 트리를 생성합니다. 이 단계는 Concurrency(동시성)를 지원하여 사용자 입력이나 애니메이션 등의 우선순위가 높은 작업을 위한 작업 중지 및 재개가 가능합니다.
	- Concurrency(동시성)란 두 개 이상의 테스크를 _동시에 지원_한다는 뜻입니다. 이는 여러 작업에 대해 일시 정지와 재가동을 반복하면서 우선순위를 기반으로 일정 시간 동안 여러 업무를 처리하는 것입니다.
 	- Concurrency는 두 개 이상의 테스크를 _동시에 실행_할 수 있는 Parallelism(평행성)과 비슷해 보이지만 엄연히 다릅니다. Parallelism을 위해서는 물리적으로 여러 개의 스레드를 사용 가능해야 하며, 자바스크립트와 같은 싱글 스레드 환경에서는 Parallelism이 불가능합니다. 
...
### Fiber 노드
Fiber 노드(Fiber Node)는 리액트의 렌더링을 관리하는 핵심 단위로, 실제 DOM 노드나 컴포넌트를 나타내며, 아래와 같은 속성을 가집니다.
...
[Medium] Concurrency vs. Parallelism — A brief view (Madhavan Nagarajan)
